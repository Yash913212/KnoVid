      ...
      <div className="pointer-events-none absolute left-4 top-4 z-10 rounded-xl border border-black/10 bg-white/70 px-3 py-2 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03]">
        ...
        <div className="mt-1.5 flex items-center gap-3 text-[10px] font-medium text-stone-500 dark:text-[#737373]">
          <span className="flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-[#2BA6A0] shadow-[0_0_6px_rgb(43_166_160/0.8)]" />
            Concept
          </span>
          <span className="flex items-center gap-1.5">
  ...
  return (
    <div ref={zoneRef} className="relative grid h-56 w-56 place-items-center" aria-hidden="true">
      ...
      <div className="relative grid place-items-center px-4 text-center">
        ...
        >
          <Sparkles size={20} className="text-[#73CEC2] dark:text-[#73CEC2]" />
        </motion.div>
        ...
        <p className="mt-1 max-w-[180px] text-[10px] leading-relaxed text-stone-400 dark:text-[#737373]">
          {near ? 'Release to fuse' : armed ? <>Pair with <span className="text-[#73CEC2]">{pending}</span></> : 'Drag two ideas together'}
        </p>
      </div>
    </div>
  )
    ...
    <div>
      <div className="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-4">
        <div className="min-w-0">
          <p className="font-display text-[10px] font-bold uppercase tracking-[0.24em] text-[#73CEC2]">Node Fusion</p>
      ...
      <div className="px-6 py-5">
        <AnimatePresence mode="wait">
          {phase === 'synth' ? (
            ...
            >
              {error ? (
                <div className="rounded-2xl border border-[#2BA6A0]/30 bg-[#2BA6A0]/[0.06] p-4 text-sm text-stone-300">
                  <p className="font-semibold text-[#73CEC2]">{error}</p>
                  <button
                    type="button"
                    onClick={() => setAttempt((n) => n + 1)}
                    className="mt-3 rounded-xl border border-[#2BA6A0]/40 bg-[#2BA6A0]/10 px-3 py-1.5 text-xs font-semibold text-[#73CEC2] transition-colors hover:bg-[#2BA6A0]/20"
                  >
                  ...
                  <>
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        type="button"
                        onClick={copy}
                        className="inline-flex items-center gap-1.5 rounded-xl border border-[#2BA6A0]/40 bg-[#2BA6A0]/10 px-3 py-1.5 text-xs font-semibold text-[#73CEC2] transition-colors hover:bg-[#2BA6A0]/20"
                      >
...
function HighlightTerm({ text, term }: { text: string; term: string }) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded-sm bg-[#2BA6A0]/20 px-0.5 text-[#155956] dark:bg-[#2BA6A0]/25 dark:text-[#B5E4D5]">
