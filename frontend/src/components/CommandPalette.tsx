            ...
            <div ref={listRef} className="max-h-[46vh] overflow-y-auto p-2">
              {loading && videos === null && (
                <div className="space-y-1.5 p-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-10 rounded-lg skeleton-shimmer dark:bg-stone-800" />
                  ))}
                </div>
              )}
              {!loading && results.length === 0 && (
                <p className="px-3 py-8 text-center text-sm text-stone-400 dark:text-stone-500">No videos match “{query}”.</p>
              )}
              {results.map((v, i) => {
                const resume = getResume(v._id)
                return (
                  <button
                    key={v._id}
                    type="button"
                  ...
                  >
                    <span className={`h-2 w-2 shrink-0 rounded-full ${STATUS_COLORS[v.status].split(' ')[0]}`} />
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block text-xs text-stone-400 dark:text-stone-500">
                        ...
                        {resume && v.status === 'done' && ` · resume ${formatTime(resume)}`}
                      </span>
                    </span>
